...
def evaluate(log_path):
    ...
    with open(log_path, "r") as f:
        for line in f:
            ...
            if entry["llm_avg_logprob"] is not None:
                entry["confidence"] = float(np.exp(entry["llm_avg_logprob"]))
            else:
    ...
    lengths = [e["llm_length"] for e in entries]
    entropies = [e["llm_avg_entropy"] for e in entries]
    logprobs = [e["llm_avg_logprob"] for e in entries]
...
if __name__ == "__main__":
    ...
    evaluate("logs/hotpot_qa_llm.jsonl")